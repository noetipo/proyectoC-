    ...
    void Trie::insert (char *word) {
        ...
        while (true) {
            ...
            if (pos == notFound) { 	// by the first letter of word
                ...
                if (strcmp(lf->word,word+1) == 0) {cout << "Duplicate entry2 " << hold << endl;
                    return;
                }
                offset = 0;
                ...
                do {
                    ...
                    if (strlen(word) == offset+1) {
                        ...
                        createLeaf(lf->word[offset],lf->word + offset+1,
                                   p->ptrs[pos]);
                        return;
                    }
                        ...
                        p->ptrs[pos]->endOfWord = true;
                        createLeaf(word[offset+1],word+offset+2,
                                   p->ptrs[pos]);
                        return;
                    }
                    p->ptrs[pos] = new TrieNonLeafNode(word[offset+1]);
    ...
    int main(int argc, char* argv[]) {
        ...
        int i, lineNum = 1;
        ifstream dictionary("/tmp/dictionary.txt");
        if (dictionary.fail()) {
            cerr << "Cannot open 'dictionary.txt'\n";
            exit(-1);
        }
        dictionary >> s;
