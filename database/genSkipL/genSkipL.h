...
#ifndef PROYECTOC_GENSKIPL_H
#define PROYECTOC_GENSKIPL_H
...
void SkipList<T>::skipListInsert (const T& key) {
    ...
    for (lvl = maxLevel - 1; lvl >= 0; lvl--) {
        ...
        newNode->snext = new nodePtr[sizeof(nodePtr) * (lvl+1)];
        newNode->key = key;
